...
<body>
    ...
    goScoper() {
        if (this.documentViewPanel) {
            ...
            this.documentViewPanel.webview.onDidReceiveMessage(
                ...
                undefined,
                exports._context.subscriptions
            );
            this.documentViewPanel.onDidDispose(
                () => {
                    this.documentViewPanel = undefined;
        ...
        try {
            this.loadDocument("http://127.0.0.1:8000");
        } catch (e) {
            console.trace(e);
        }
    }
    openGoScoperFolder() {
        ...
        const arch = os.arch(); // 获取硬件架构
        let goScoperPath = path.join(exports._context.extensionPath, "goScoper");
        let sPath = "";
        if (platform == "win32") {
            sPath = path.join(goScoperPath, "windows_amd64");
            exec(`explorer "${sPath}"`);
        } else if (platform == "darwin") {
            if (arch == "arm64") {
                sPath = path.join(goScoperPath, "darwin_arm64");
            } else if (arch == "x64" || arch == "amd64") {
                sPath = path.join(goScoperPath, "darwin_amd64");
            }
            exec(`open "${sPath}"`);
        } else if (platform == "linux" && arch == "amd64") {
            sPath = path.join(goScoperPath, "linux_amd64");
            exec(`xdg-open "${sPath}"`);
        }
        vscode.window.showInformationMessage("尝试打开 goScoper 文件夹");
    }

    private loadDocument(url: string) {
        // 明确 url 参数的类型为 string
        try {
    ...
    captureToGoScoper() {
        server.sendCommand("captureToGoScoper", {
            id: "__capture_to_goscoper__",
            name: "__capture_to_goscoper__",
            script: "",
        });
    }
...
const commands = ["startAllServer", "stopAllServer", "startServer", "stopServer", "startTrackADBDevices", "stopTrackADBDevices", "manuallyConnectADB", "manuallyDisconnect", "showServerAddress", "showQrCode", "openDocument", "run", "runOnDevice", "stop", "stopAll", "rerun", "save", "saveToDevice", "newProject", "runProject", "saveProject", "screenshotToPC", "goScoper", "captureToGoScoper", "openGoScoperFolder"];
